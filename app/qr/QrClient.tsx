...
function QrCanvas({ text, size = 256 }: { text: string; size?: number }) {
...
function QrCard({
  ...
  return (
    <div className="p-4 border rounded flex flex-col items-center gap-3 print:break-inside-avoid">
      ...
      {/* asocia el canvas con data-url para la descarga */}
      <div className="relative">
        <QrCanvas text={url} size={size} />
        {/* marcaremos el canvas con data-url después de pintar */}
        <script
          dangerouslySetInnerHTML={{
            __html: `
              (function(){
                var cs = document.currentScript.previousElementSibling?.querySelector('canvas');
                if(cs){ cs.setAttribute('data-url', ${JSON.stringify(url)}); }
              })();
            `,
          }}
        />
      </div>
      <div className="text-xs break-all text-center max-w-[${size}px]">{url}</div>
...
export default function QrClient({
  ...
  const [showAll, setShowAll] = useState<boolean>(!!initialRoomId ? false : true);
  const makeUrl = (r: string) => `${origin}/scan?roomId=${encodeURIComponent(r)}`;
  const singleUrl = useMemo(() => makeUrl(room), [room, origin]);
    ...
    <div className="p-6 space-y-6">
      <div className="no-print">
        ...
        <p className="text-sm opacity-70">
          Escanear redirige a <code>/teacher/sessions/&lt;id&gt;</code> de la sesión vigente del salón.
        </p>
      </div>
      ...
      <div className="no-print border rounded p-4 space-y-3">
        ...
        {room && <QrCard label={room} url={singleUrl} />}
      </div>

      {/* Hoja con todos (o desactivar) */}
      <div className="no-print border rounded p-4 space-y-3">
      ...
      {/* Estilos para impresión: ocultar controles con .no-print */}
      <style>{`
        @media print {
